...
[[maybe_unused]] static void generate_tangents(void* verts, const egkr::vector<uint32_t>& indices)
{
	...
	egkr::float3 tangent = {(fc * (deltaV2 * edge1.x - deltaV1 * edge2.x)), (fc * (deltaV2 * edge1.y - deltaV1 * edge2.y)), (fc * (deltaV2 * edge1.z - deltaV1 * edge2.z))};
...
template <typename vertex> [[maybe_unused]] static void generate_tangents(egkr::vector<vertex>& vertices, const egkr::vector<uint32_t>& indices)
{
    for (auto i{0U}; i < indices.size(); i += 3)
    {
	auto i0 = indices[i + 0];
