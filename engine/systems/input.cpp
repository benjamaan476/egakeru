...
namespace egkr
{
    ...
    bool input::is_key_down(key key) { return state->current_keyboard.keys[(size_t)key]; }

    bool input::is_key_up(key key) { return !is_key_down(key); }

    bool input::was_key_down(key key) { return state->previous_keyboard.keys[(size_t)key]; }

    bool input::was_key_up(key key) { return !was_key_down(key); }

    bool input::was_key_pressed(key key) { return was_key_up(key) && is_key_down(key); }

    bool input::was_key_released(key key) { return was_key_down(key) && is_key_up(key); }
    ...
    bool input::is_button_up(mouse_button button) { return !is_button_down(button); }

    bool input::is_button_down(mouse_button button) { return state->current_mouse.buttons[(size_t)button]; }

    bool input::is_button_dragging(mouse_button button) { return state->current_mouse.dragging[(size_t)button]; }

    bool input::was_button_up(mouse_button button) { return !was_button_down(button); }

    bool input::was_button_down(mouse_button button) { return state->previous_mouse.buttons[(size_t)button]; }
    ...
    int2 input::get_mouse_position() { return {state->current_mouse.x, state->current_mouse.y}; }

    int2 input::get_previous_mouse_position() { return {state->previous_mouse.x, state->previous_mouse.y}; }
    ...
    void input::push_keymap(const keymap& keymap) { state->keymaps.push_back(keymap); }

    void input::pop_keymap() { state->keymaps.pop_back(); }
