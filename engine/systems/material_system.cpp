...
namespace egkr
{
	...
	bool material_system::load_material(const material::properties& properties, material::shared_ptr& material)
	{
		...
		if (properties.material_type == material::type::pbr)
		{

			texture_map::properties albedo_properties{ .minify = texture_map::filter::linear,
				.magnify = texture_map::filter::linear,
				.repeat_u = texture_map::repeat::repeat,
				.repeat_v = texture_map::repeat::repeat,
				.repeat_w = texture_map::repeat::repeat,
				.map_use = texture_map::use::map_albedo };
			auto albedo_map = texture_map::texture_map::create(albedo_properties);
			albedo_map->map_texture = texture_system::acquire(properties.albedo_map_name);
			if (albedo_map->map_texture == nullptr)
			{
				LOG_WARN("Failed to find texture: {} for material {}. Setting default", properties.albedo_map_name.data(), properties.name.data());
				albedo_map->map_texture = texture_system::get_default_albedo_texture();
			}
			albedo_map->acquire();
			maps.push_back(albedo_map);
			material->set_albedo_map(albedo_map);

			auto metallic_map = texture_map::texture_map::create(albedo_properties);
			metallic_map->map_texture = texture_system::acquire(properties.metallic_map_name);
			if (metallic_map->map_texture == nullptr)
			{
				LOG_WARN("Failed to find texture: {} for material {}. Setting default", properties.metallic_map_name.data(), properties.name.data());
				metallic_map->map_texture = texture_system::get_default_metallic_texture();
			}
			metallic_map->acquire();
			maps.push_back(metallic_map);
			material->set_metallic_map(metallic_map);

			auto roughness_map = texture_map::texture_map::create(albedo_properties);
			roughness_map->map_texture = texture_system::acquire(properties.roughness_map_name);
			if (roughness_map->map_texture == nullptr)
			{
				LOG_WARN("Failed to find texture: {} for material {}. Setting default", properties.roughness_map_name.data(), properties.name.data());
				roughness_map->map_texture = texture_system::get_default_roughness_texture();
			}
			roughness_map->acquire();
			maps.push_back(roughness_map);
			material->set_roughness_map(roughness_map);

			auto ao_map = texture_map::texture_map::create(albedo_properties);
			ao_map->map_texture = texture_system::acquire(properties.ao_map_name);
			if (ao_map->map_texture == nullptr)
			{
				LOG_WARN("Failed to find texture: {} for material {}. Setting default", properties.ao_map_name.data(), properties.name.data());
				ao_map->map_texture = texture_system::get_default_ao_texture();
			}
			ao_map->acquire();
			maps.push_back(ao_map);
			material->set_ao_map(ao_map);
		}
