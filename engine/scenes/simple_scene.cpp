...
namespace egkr::scene
{
	...
	if (state_ >= state::loaded)
	{
	    auto frustum = egkr::frustum(camera->get_position(), camera->get_forward(), camera->get_right(), camera->get_up(), viewport->viewport_rect.w / viewport->viewport_rect.z, viewport->fov,
