...
namespace egkr
{
	...
	ray ray::from_screen(const int2& screen_position, const float4& viewport_rect, const float3& origin, const float4x4& view, const float4x4& projection)
	{
		...
		float2 ndc{};
		ndc.x = 2.f * ((float)screen_position.x - (float)viewport_rect.x) / viewport_rect.z - 1.f;
		ndc.y = 1.f - 2.f * ((float)screen_position.y - (float)viewport_rect.y) / viewport_rect.w;
