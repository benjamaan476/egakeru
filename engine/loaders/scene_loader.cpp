...
namespace egkr
{
	...
	*(scene::configuration*)properties.data = scene_configuration;
	...
	scene::terrain_scene_configuration current_terrain{};
	...
	for (; !line.empty(); line = filesystem::read_line(handle, 511), ++line_number)
	{
		...
		parse_mode = parse_mode::terrain;
		current_terrain = {};
	    }
	    else if (variable_name == "[/Terrain]")
	    {
		if (parse_mode != parse_mode::terrain)
		{
		    LOG_ERROR("Cannot leave terrain parsing from non-terrain parse mode. Mismatching tag");
		    return {};
		}
		parse_mode = parse_mode::root;
		configuration.terrains.push_back(current_terrain);
	    }
