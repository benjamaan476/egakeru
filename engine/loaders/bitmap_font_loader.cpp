...
namespace egkr
{
    bitmap_font_loader::unique_ptr bitmap_font_loader::create(const loader_properties& properties) { return std::make_unique<bitmap_font_loader>(properties); }

    bitmap_font_loader::bitmap_font_loader(const loader_properties& properties): resource_loader(resource::type::bitmap_font, properties) { }

    resource::shared_ptr bitmap_font_loader::load(const std::string& name, void* /*params*/)
    {
	auto base_path = get_base_path();

	const std::array<supported_bitmap_font_file_type, 2> filetypes{
	    supported_bitmap_font_file_type{".ebf", bitmap_font_file_type::ebf, true}, supported_bitmap_font_file_type{".fnt", bitmap_font_file_type::fnt, false}};
	...
	resource::properties properties{
	    .type = resource::type::bitmap_font,
	    .name = name.data(),
	    .full_path = filename,
	};
	...
	*((font::bitmap_font_resource_data*)(properties.data)) = resource_data;
	...
	if (resource->data)
	{
	    delete (font::bitmap_font_resource_data*)resource->data;
