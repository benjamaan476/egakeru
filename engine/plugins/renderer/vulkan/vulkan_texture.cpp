...
namespace egkr
{
	...
	subresource.setBaseMipLevel(0).setBaseArrayLayer(0).setLevelCount(properties_.mip_levels).setAspectMask(view_properties.aspect_flags);
    ...
    vulkan_texture::vulkan_texture(const vulkan_context* context, uint32_t width, uint32_t height, const egkr::texture::properties& texture_properties)
        : texture(texture_properties), context_{context}, width_{width}, height_{height}, mip{texture_properties.mip_levels}
    {
    }
	...
	//generate_mipmaps(texture_properties);
    }
    ...
    bool vulkan_texture::generate_mipmaps(const command_buffer& command_buffer, vk::Format image_format, uint32_t mip_levels)
    {
	if (properties_.mip_levels <= 1)
	{
	    LOG_WARN("Invalid mip level provided: {}", properties_.mip_levels);
	    return false;
	}

	vk::FormatProperties format_properties = context_->device.physical_device.getFormatProperties(image_format);
	if (!(vk::FormatFeatureFlagBits::eSampledImageFilterLinear & format_properties.optimalTilingFeatures))
	{
	    LOG_WARN("Device does not support linear blitting, cannot create mip maps");
	    return false;
	}

	vk::ImageSubresourceRange subresource;
	subresource.setAspectMask(vk::ImageAspectFlagBits::eColor).setBaseArrayLayer(0).setLayerCount(1).setLevelCount(1);

	vk::ImageMemoryBarrier barrier;
	barrier.setImage(image_).setSrcQueueFamilyIndex(context_->device.graphics_queue_index).setDstQueueFamilyIndex(context_->device.graphics_queue_index).setSubresourceRange(subresource);

	int32_t mip_width = (int32_t)width_;
	int32_t mip_height = (int32_t)height_;
	...
	for (uint32_t i = 1; i < mip_levels; i++)
	{
	    ...
	    command_buffer.get_handle().pipelineBarrier(vk::PipelineStageFlagBits::eTransfer, vk::PipelineStageFlagBits::eTransfer, {}, {}, {}, barrier);

	    vk::ImageSubresourceLayers srcSubresource;
	    srcSubresource.setAspectMask(vk::ImageAspectFlagBits::eColor).setMipLevel(i - 1).setBaseArrayLayer(0).setLayerCount(1);

	    vk::ImageSubresourceLayers dstSubresource;
	    dstSubresource.setAspectMask(vk::ImageAspectFlagBits::eColor).setMipLevel(i).setBaseArrayLayer(0).setLayerCount(1);
	    ...
	    command_buffer.get_handle().blitImage(image_, vk::ImageLayout::eTransferSrcOptimal, image_, vk::ImageLayout::eTransferDstOptimal, blit, vk::Filter::eLinear);
	    ...
	    command_buffer.get_handle().pipelineBarrier(vk::PipelineStageFlagBits::eTransfer, vk::PipelineStageFlagBits::eFragmentShader, {}, {}, {}, barrier);
	...
	if (texture_data)
	{
	    ...
	    const vulkan_texture::properties image_properties{.image_format = image_format,
	        ...
	        .mip_levels = texture_properties.mip_levels};
	...
	if (mip <= 1 || !generate_mipmaps(single_use, image_format, properties_.mip_levels))
	{
	...
	subresource_range.setBaseMipLevel(0).setLevelCount(properties_.mip_levels).setBaseArrayLayer(0).setAspectMask(vk::ImageAspectFlagBits::eColor);
    ...
    vk::Sampler vulkan_texture_map::create_sampler()
    {
	mip_levels = map_texture->get_mips();
	...
	create_info.setMinFilter(convert_filter_type("min", minify))
	    ...
	    .setMinLod(0)
	    //.setMinLod(map_texture->get_mips() > 1 ? map_texture->get_mips() : 0)
	    .setMaxLod(map_texture->get_mips());

	return context_->device.logical_device.createSampler(create_info, context_->allocator);
    }

    void vulkan_texture_map::acquire()
    {
	sampler = create_sampler();
	SET_DEBUG_NAME(context_, VkObjectType::VK_OBJECT_TYPE_SAMPLER, (uint64_t)(VkSampler)sampler, map_texture->get_name() + "sampler");
    }
    ...
    bool vulkan_texture_map::refresh()
    {
	if (map_texture && map_texture->data)
	{
	    vk::Sampler new_sampler = create_sampler();

	    auto old_sampler = sampler;

	    context_->device.logical_device.waitIdle();

	    sampler = new_sampler;

	    context_->device.logical_device.destroySampler(old_sampler);
	}
