...
#include "pch.h"
#include "plugins/renderer/vulkan/vulkan_texture.h"
...
namespace egkr
{
    ...
    bool vulkan_device::physical_device_meets_requirements(vk::PhysicalDevice device, vk::SurfaceKHR surface, const vk::PhysicalDeviceProperties& device_properties,
        const vk::PhysicalDeviceFeatures device_features, const physical_device_requirements& requirements, physical_device_queue_family_info& family_info, swapchain_support_details& swapchain_support)
    {
	ZoneScoped;
	...
	LOG_INFO("       {:d}|       {:d}|       {:d}|       {:d}| {}", family_info.graphics_index != invalid_32_id, family_info.present_index != invalid_32_id, family_info.compute_index != invalid_32_id,
	    family_info.transfer_index != invalid_32_id, device_properties.deviceName.data());

	if ((!requirements.graphics || (requirements.graphics && family_info.graphics_index != invalid_32_id)) && (!requirements.present || (requirements.present && family_info.present_index != invalid_32_id))
	...
	for (auto i{0U}; i < index_count; ++i)
	{
	    ...
	    device_queue_create_infos[i].setQueueFamilyIndex(indices[i]).setQueueCount(1).setPQueuePriorities(&queue_priority);
	}
	...
	device_create_info.setQueueCreateInfos(device_queue_create_infos).setQueueCreateInfoCount(index_count).setPEnabledFeatures(&device_features).setPEnabledExtensionNames(extension_names);
	...
	pool_create_info.setQueueFamilyIndex(graphics_queue_index).setFlags(vk::CommandPoolCreateFlagBits::eResetCommandBuffer);
	...
	submit_info.setCommandBuffers(command_buffer.get_handle()).setWaitDstStageMask(stage_mask);
	if (draw_index_ == 0)
	{
	    submit_info.setSignalSemaphores(context_.queue_complete_semaphore[context_.current_frame]).setWaitSemaphores(context_.image_available_semaphore[context_.current_frame]);
	}
	else
	{
	    submit_info.setSignalSemaphoreCount(0).setWaitSemaphoreCount(0);
	}
	...
	application_info.setPApplicationName("engine")
	...
	create_info.messageSeverity = VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT;
	...
	return (properties.deviceType == vk::PhysicalDeviceType::eIntegratedGpu || properties.deviceType == vk::PhysicalDeviceType::eDiscreteGpu) && (bool)features.geometryShader && queueIndices.is_complete()
	    && swapChainAdequate;
    }
	...
	for (auto queue_family : unique_queue_familes)
	{
	    ...
	    device_queue_create_info.setQueueFamilyIndex(queue_family).setQueueCount(1).setQueuePriorities(queue_priority);
	...
	vk::DeviceCreateInfo device_create_info{};
	device_create_info.setQueueCreateInfos(device_queue_create_infos).setPEnabledFeatures(&physical_device_features).setEnabledExtensionCount((uint32_t)device_extensions_.size());
	...
	create_pool.setQueueFamilyIndex(device.graphics_queue_index).setFlags(vk::CommandPoolCreateFlagBits::eResetCommandBuffer);
	...
	auto tex = vulkan_texture::create(&context_, properties.width, properties.height, properties);
	...
	*(vulkan_texture*)out_texture = vulkan_texture::create_raw(&context_, properties.width, properties.height, properties);
    ...
    render_target::render_target::shared_ptr renderer_vulkan::create_render_target(
    ...
    texture_map::shared_ptr renderer_vulkan::create_texture_map(const texture_map::properties& properties) const { return vulkan_texture_map::create(&context_, properties); }

    renderbuffer::renderbuffer::shared_ptr renderer_vulkan::create_renderbuffer(renderbuffer::type buffer_type, uint64_t size) const { return vulkan_buffer::create(&context_, buffer_type, size); }

    void renderer_vulkan::set_viewport(const float4& rect) const
    {
	vk::Viewport viewport{};
	viewport.setX(rect.x).setY(rect.y).setWidth(rect.z).setHeight(rect.w).setMinDepth(0.F).setMaxDepth(1.F);
    ...
    {
	vk::Rect2D scissor{};
	scissor.setOffset({(int32_t)rect.x, (int32_t)rect.y}).setExtent({(uint32_t)rect.z, (uint32_t)rect.w});

	auto& command_buffer = context_.graphics_command_buffers[context_.image_index];
	command_buffer.get_handle().setScissor(0, scissor);
    }

    void renderer_vulkan::reset_scissor() const { set_scissor(context_.scissor_rect); }
    ...
    uint32_t renderer_vulkan::get_window_attachment_count() const { return context_.swpchain->get_image_count(); }

    texture::shared_ptr renderer_vulkan::get_window_attachment(uint8_t index) const
    {
    ...
    texture::shared_ptr renderer_vulkan::get_depth_attachment(uint8_t index) const { return context_.swpchain->get_depth_attachment(index); }

    uint8_t renderer_vulkan::get_window_index() const { return (uint8_t)context_.image_index; }
