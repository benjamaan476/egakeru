...
namespace egkr
{
	namespace audio
	{
		...
		public:
			...
			[[nodiscard]] virtual float3 get_listener_position() const = 0;
			virtual bool set_listener_position(const float3& position) = 0;

			[[nodiscard]] virtual std::pair<const float3&, const float3&> get_orientation() const = 0;
			virtual bool set_orientation(const float3& forward, const float3& up) = 0;

			[[nodiscard]] virtual float get_source_gain(uint32_t source_id) const = 0;
			virtual bool set_source_gain(uint32_t source_id, float gain) = 0;

			[[nodiscard]] virtual float get_source_pitch(uint32_t source_id) const = 0;
			virtual bool set_source_pitch(uint32_t source_id, float pitch) = 0;

			[[nodiscard]] virtual bool get_looping(uint32_t source_id) const = 0;
			virtual bool set_looping(uint32_t source_id, bool loop) = 0;

			[[nodiscard]] virtual const float3& get_source_position(uint32_t source_id) const = 0;
