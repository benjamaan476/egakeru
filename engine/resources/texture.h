...
namespace egkr
{
	...
	void set_flags(flags texture_flags) { properties_.texture_flags = texture_flags; }
	void set_width(uint32_t width) { properties_.width = width; }
	void set_height(uint32_t height) { properties_.height = height; }
	void set_channel_count(uint8_t channel_count) { properties_.channel_count = channel_count; }
	void set_type(type texture_type) { properties_.texture_type = texture_type; }

	[[nodiscard]] const auto& get_flags() const { return properties_.texture_flags; }
	...
	[[nodiscard]] const auto& get_generation() const { return generation; }
	[[nodiscard]] const auto& get_mips() const { return mip_levels; }
