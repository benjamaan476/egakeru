...
namespace egkr
{
	...
	bool scene::init()
	{
		...
		terrain_shader_locations.material_location = terrain_shader->get_uniform_index("materials");
		...
		terrain_shader_locations.num_materials = terrain_shader->get_uniform_index("num_materials");

		terrain_shader_locations.samplers[0] = terrain_shader->get_uniform_index("diffuse_sampler_0");
		terrain_shader_locations.samplers[1] = terrain_shader->get_uniform_index("specular_sampler_0");
		terrain_shader_locations.samplers[2] = terrain_shader->get_uniform_index("normal_sampler_0");
		terrain_shader_locations.samplers[3] = terrain_shader->get_uniform_index("diffuse_sampler_1");
		terrain_shader_locations.samplers[4] = terrain_shader->get_uniform_index("specular_sampler_1");
		terrain_shader_locations.samplers[5] = terrain_shader->get_uniform_index("normal_sampler_1");
		terrain_shader_locations.samplers[6] = terrain_shader->get_uniform_index("diffuse_sampler_2");
		terrain_shader_locations.samplers[7] = terrain_shader->get_uniform_index("specular_sampler_2");
		terrain_shader_locations.samplers[8] = terrain_shader->get_uniform_index("normal_sampler_2");
		terrain_shader_locations.samplers[9] = terrain_shader->get_uniform_index("diffuse_sampler_3");
		terrain_shader_locations.samplers[10] = terrain_shader->get_uniform_index("specular_sampler_3");
		terrain_shader_locations.samplers[11] = terrain_shader->get_uniform_index("normal_sampler_3");
	    }
	...
	bool scene::execute(const frame_data& frame_data) const
	{
		...
		for (const auto& terrain : data.terrain)
		{
		    ...
		    std::vector<material_info> info(4);
		    for (const auto& [i, mat] : terrain.materials | std::views::enumerate)
		    {
			uint32_t index = (uint32_t)i;
			info[index].diffuse_colour = mat->get_diffuse_colour();
			info[index].shininess = mat->get_shininess();

			shader_system::set_uniform(terrain_shader_locations.samplers[3 * index + 0], &mat->get_diffuse_map());
			shader_system::set_uniform(terrain_shader_locations.samplers[3 * index + 1], &mat->get_specular_map());
			shader_system::set_uniform(terrain_shader_locations.samplers[3 * index + 2], &mat->get_normal_map());
		    }

		    shader_system::set_uniform(terrain_shader_locations.material_location, info.data());

		    shader_system::apply_global(true);
		    if (terrain.is_winding_reversed)
		    {
			engine::get()->get_renderer()->set_winding(winding::clockwise);
		    }

		    terrain.render_geometry->draw();
