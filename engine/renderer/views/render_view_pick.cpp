...
namespace egkr::render_view
{
	...
	bool render_view_pick::on_create()
	{
		...
		std::string ui_shader_name = "Shader.Builtin.UIPick";
		...
		shader_system::create_shader(*shader, ui_shader_info.renderpass.get());
		resource_system::unload(resource);

		ui_shader_info.shader = shader_system::get_shader(ui_shader_name);
		...
		const std::string world_shader_name = "Shader.Builtin.UIWorld";
		auto world_resource = resource_system::load(world_shader_name, resource_type::shader, nullptr);
		auto world_shader = (shader::properties*)resource->data;
		shader_system::create_shader(*world_shader, world_shader_info.renderpass.get());
		resource_system::unload(resource);

		world_shader_info.shader = shader_system::get_shader(world_shader_name);
	...
	bool render_view_pick::on_render(const render_view_packet* render_view_packet, uint32_t frame_number, uint32_t render_target_index) const
	{
		//Only do this every three frames
		if (render_target_index == 0)
		{
			auto* packet_data = (pick_packet_data*)render_view_packet->extended_data;
			...
			for (auto i{ 0U }; i < packet_data->world_mesh_data.meshes.size(); ++i)
			{
			...
			for (auto i{ packet_data->world_mesh_data.meshes.size() }; i < render_view_packet->render_data.size(); ++i)
			{
				auto& render_data = render_view_packet->render_data[i];
	...
	bool render_view_pick::regenerate_attachment_target(uint32_t pass_index, const render_target::attachment& attachment)
	{
		return false;
	}
	...
	{
		uint32_t instance = ui_shader_info.shader->acquire_instance_resources({});
		instance = world_shader_info.shader->acquire_instance_resources({});
