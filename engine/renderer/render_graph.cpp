...
namespace egkr
{
	bool rendergraph::pass::init()
	{
		return false;
	}
	bool rendergraph::pass::execute(const frame_data& /*frame_data*/) const
	{
		return false;
	}
	...
	rendergraph::rendergraph(const std::string& rendergraph_name, const application* application)
		:name{rendergraph_name}, app{application}
	{
	}

	rendergraph rendergraph::create(const std::string& rendergraph_name, application* application)
	{
		return rendergraph(rendergraph_name, application);
	}
	...
	bool rendergraph::create_pass(const std::string& pass_name, std::function<bool(pass*)> init)
	{
		for (const auto& pass : passes)
		{
	...
	bool rendergraph::finalise()
	{
		for (auto& global_source : global_sources)
		{
			if (global_source.source_origin == source::origin::global)
			{
				...
				{
					if (global_source.source_type == source::type::reder_target_colour)
					{
						global_source.textures.push_back(renderer->get_backend()->get_window_attachment(i));
					}
					else if (global_source.source_type == source::type::reder_target_depth_stencil)
					{
