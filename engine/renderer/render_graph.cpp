...
namespace egkr
{
    ...
    bool rendergraph::set_sink_linkage(const std::string& pass_name, const std::string& sink_name, const std::string& source_pass_name, const std::string& source_name)
    {
        ...
        else
        {
            ...
            if(source_pass == passes.end())
            {
                LOG_FATAL("Invalid render graph. Attempted to link to a pass that doesn't exist: {}", source_pass_name);
                return false;
            }
            auto src = std::ranges::find_if((*source_pass)->get_sources(), [source_name](const auto& s) { return s.name == source_name; });
            if(src == (*source_pass)->get_sources().end())
            {
                LOG_FATAL("Invalid render graph. Attempted to link to a source that does not exist: {}", source_name);
                return false;
            }
    ...
    bool rendergraph::finalise()
    {
        for (auto& global_source : global_sources)
        {
            ...
            {
                LOG_FATAL("Invalid render graph. Global source not marked as a global origin");
                return false;
            }
        }
        ...
        for (const auto& renderpass : passes)
        {
            ...
            {
                const auto& src = snk.bound_source;
                if(!src)
                {
                    LOG_WARN("Unused sink in render graph: {}", snk.name);
                }
                else if (src->source_origin == source::origin::global && src->source_type == source::type::render_target_colour)
                {
        ...
        for (auto& renderpass : passes)
        {
            ...
            {
                // If source of renderpass comes from another pass
