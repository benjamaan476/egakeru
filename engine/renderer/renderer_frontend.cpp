...
namespace egkr
{
    ...
    void renderer_frontend::free_material(material* texture) const { return backend_->free_material(texture); }

    texture* renderer_frontend::create_texture() const { return backend_->create_texture(); }

    texture* renderer_frontend::create_texture(const texture::properties& properties, const uint8_t* data) const { return backend_->create_texture(properties, data); }
